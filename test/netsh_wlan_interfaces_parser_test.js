...
describe('netsh_wlan_interface_parser', function() {
  ...
  it('should display correct interface description', function *() {
    assert.strictEqual(info.wireless_network_connection.description, 'Intel(R) Centrino(R) Ultimate-N 6300 AGN');
  });

  it('should display correct interface GUID', function *() {
    assert.strictEqual(info.wireless_network_connection.guid, 'c120524d-ecfd-40f5-91cf-70b301342d42');
  });

  it('should display correct physical address', function *() {
    assert.strictEqual(info.wireless_network_connection.physical_address, '91:5b:ce:af:f9:e6');
  });

  it('should display correct interface state', function *() {
    assert.strictEqual(info.wireless_network_connection.state, 'connected');
  });

  it('should display correct SSID', function *() {
    assert.strictEqual(info.wireless_network_connection.ssid, 'test01');
  });

  it('should display correct BSSID', function *() {
    assert.strictEqual(info.wireless_network_connection.bssid, 'f9:bd:d1:5e:f8:66');
  });

  it('should display correct network type', function *() {
    assert.strictEqual(info.wireless_network_connection.network_type, 'Infrastructure');
  });

  it('should display correct radio type', function *() {
    assert.strictEqual(info.wireless_network_connection.radio_type, '802.11n');
  });

  it('should display correct authentication', function *() {
    assert.strictEqual(info.wireless_network_connection.authentication, 'WPA2-Enterprise');
  });

  it('should display correct cipher', function *() {
    assert.strictEqual(info.wireless_network_connection.cipher, 'CCMP');
  });

  it('should display correct channel', function *() {
    assert.strictEqual(info.wireless_network_connection.channel, '36');
  });

  it('should display correct receive rate', function *() {
    assert.strictEqual(info.wireless_network_connection.receive_rate, '300');
  });

  it('should display correct transmit rate', function *() {
    assert.strictEqual(info.wireless_network_connection.transmit_rate, '300');
  });

  it('should display correct signal', function *() {
    assert.strictEqual(info.wireless_network_connection.signal, '99%');
  });

  it('should display correct profile', function *() {
    assert.strictEqual(info.wireless_network_connection.profile, 'test01');
  });

  it('should display correct hosted network status', function *() {
    assert.strictEqual(info.wireless_network_connection.hosted_network_status, 'Not started');
  });
});
